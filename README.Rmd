...
Now we feed our function to the `immosaic()` function, which packs the generated images onto a canvas: 
...
```{r send_tweet, echo=FALSE, results='hide'}
auth <- rtweet::rtweet_bot(api_key = Sys.getenv("TWITTER_AP_KEY"),
...
rtweet::auth_as(auth)
rtweet::post_tweet("A packed image mosaic of @PhyloPic images generated with the {immosaic} package (https://github.com/rdinnager/immosaic). If you like this image please visit http://phylopic.org/ to check out more images and consider supporting them with a donation. Artists showcased: https://github.com/rdinnager/immosaic#artists-whose-work-is-showcased", 
